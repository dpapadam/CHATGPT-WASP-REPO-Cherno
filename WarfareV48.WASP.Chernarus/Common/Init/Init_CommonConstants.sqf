...
//--- Economy.
	...
	if (isNil "WFBE_C_ECONOMY_FUNDS_START_GUER") then {WFBE_C_ECONOMY_FUNDS_START_GUER = if (WF_Debug) then {900000} else {409600}};
...
//--- Structures.
	...
	WFBE_C_STRUCTURES_COMMANDCENTER_RANGE = 7500; //--- Command Center Range.
...
//--- Towns.
	...
	WFBE_C_TOWNS_PATROL_HOPS = 3; //--- Amount of Waypoints given to the AI Patrol in towns (Higher is wider).
	...
	WFBE_C_TOWNS_UNITS_INACTIVE = 600; //--- Remove units in town if no enemies are to be found within that time.
	...
	WFBE_C_TOWNS_UNITS_WAYPOINTS = 4;
...
//--- Units.
	...
	if (isNil "WFBE_C_UNITS_CLEAN_TIMEOUT") then {WFBE_C_UNITS_CLEAN_TIMEOUT = 600}; //--- Lifespan of a dead body.
