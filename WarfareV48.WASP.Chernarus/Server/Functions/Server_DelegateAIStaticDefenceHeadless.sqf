...
if (isNil "WFBE_CURRENT_CLIENT_INDEX") then {
	WFBE_CURRENT_CLIENT_INDEX = 0; // Start at the first client
};
...
for '_i' from 0 to count(_groups) -1 do {
	...
	if (count _clients > 0) then {
		...
		_currentClientIndex = WFBE_CURRENT_CLIENT_INDEX;
		...
		WFBE_CURRENT_CLIENT_INDEX = (_currentClientIndex + 1) mod (count _clients);
	};
